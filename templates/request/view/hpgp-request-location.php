 ...
 *
 * Override this file by copying it to "hivepress/request/view/hpgp-request-location.php" in your theme.
 *
 * @package GeolocationPlus\Templates
 *
 * @var \HivePress\Models\Request $request The request being displayed.
 */
...
if ( $request->get_location() ) :
	...
	$hpgp_address = hivepress()->hpgp_geolocation ? hivepress()->hpgp_geolocation->format_address( $request->get_location(), 'request' ) : $request->get_location();
	...
	<div class="hp-request__location hp-listing__location">
		...
		<?php else : ?>
			<a href="<?php echo esc_url( $hpgp_url ); ?>" target="_blank" title="<?php echo esc_attr( $request->get_location() ); ?>"><?php echo esc_html( $hpgp_address ); ?></a>
		<?php endif; ?>
	</div>
	<?php
endif;
