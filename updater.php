 ...
 */
const UPDATE_RATE_LIMIT_KEY = 'hpgp_github_release_rate_limit';
...
function fetch_release_data() {
	...
	if ( isset( $site['reason'] ) && 'no_release' === $site['reason'] ) {
		set_site_transient( CHECK_REASON_KEY, 'no_release', HOUR_IN_SECONDS );
...
function fetch_release_from_api() {
	...
	if ( get_site_transient( UPDATE_RATE_LIMIT_KEY ) ) {
		set_site_transient( CHECK_REASON_KEY, 'rate_limited', HOUR_IN_SECONDS );
	...
	if ( 200 !== $code ) {
		...
		set_site_transient( CHECK_REASON_KEY, $reason, HOUR_IN_SECONDS );
...
function handle_update_check() {
	...
	$reason = get_site_transient( CHECK_REASON_KEY );
...
function show_update_check_notice() {
	...
	if ( 'available' === $status ) {
		...
		$message = __( 'GitHub limits how many update checks one server may make each hour, and this server has reached that limit. Nothing is wrong with the plugin or your site, and checking will work again within the hour.', 'geolocation-plus-for-hivepress' );
...
function inject_update( $transient ) {
	...
	$basename = plugin_basename( HPGP_FILE );
...
function add_bulk_check( $actions ) {
	if ( current_user_can( 'update_plugins' ) ) {
		$actions['hpx_check_updates'] = __( 'Check for updates', 'geolocation-plus-for-hivepress' );
	}
...
function handle_bulk_check( $redirect, $action, $plugin_files ) {
	if ( 'hpx_check_updates' === $action && current_user_can( 'update_plugins' ) && in_array( plugin_basename( HPGP_FILE ), (array) $plugin_files, true ) ) {
		get_latest_release( true );
	}
...
function show_bulk_check_notice() {
	...
	if ( $available ) {
		...
		$message = sprintf( _n( 'Checked %1$s plugin for updates: %2$s can be updated.', 'Checked %1$s plugins for updates: %2$s can be updated.', $checked, 'geolocation-plus-for-hivepress' ), number_format_i18n( $checked ), number_format_i18n( $available ) );
	} else {
		/* translators: %s: number of plugins checked. */
		$message = sprintf( _n( 'Checked %s plugin for updates: it is up to date.', 'Checked %s plugins for updates: all are up to date.', $checked, 'geolocation-plus-for-hivepress' ), number_format_i18n( $checked ) );
	}
...
if ( empty( $GLOBALS['hpx_update_check_bulk'] ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
	$GLOBALS['hpx_update_check_bulk'] = 'geolocation-plus-for-hivepress'; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
